...
```mermaid
flowchart TB
    Internet([Internet / Attaquants])
    Discord([Discord Webhooks - Alerting])
    Cloud([Stockage distant - S3 / Backblaze B2])

    subgraph HOST[Freebox Ultra - Debian Bookworm ARM64]
        direction TB
        FW[Pare-feu noyau - nftables / ipset]

        subgraph FRONT[frontend_net]
            NPM[Nginx Proxy Manager - 80 / 443 / 81]
        end

        subgraph BACK[backend_net]
            DBNPM[(MariaDB - db-npm)]
            CS[CrowdSec - Agent IPS/IDS + Bouncer]
            COWRIE[Cowrie - Honeypot SSH 2222]
            META[Metabase - Threat Intel BI 3008]
            DUP[Duplicati - Backup 3-2-1 8200]
        end

        subgraph MON[monitoring_net]
            PROM[Prometheus - TSDB]
            GRAF[Grafana - Dashboards 3000]
            NODE[Node Exporter - Metriques hote]
            CADV[cAdvisor - Metriques conteneurs]
            KUMA[Uptime Kuma - 3001]
        end
    end

    Internet -->|HTTP / HTTPS| FW
    FW --> NPM
    Internet -->|SSH leurre 2222| COWRIE

    NPM --> DBNPM
    NPM -. logs .-> CS
    COWRIE -. logs .-> CS
    CS -->|decisions de ban| FW
    CS -. SQLite lecture seule .-> META

    NODE --> PROM
    CADV --> PROM
    PROM --> GRAF

    KUMA -->|sondes externes| NPM
    KUMA -->|alertes| Discord
    CS -->|alertes| Discord

    DUP -->|chiffre AES-256| Cloud
```
